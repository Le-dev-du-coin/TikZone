...
export default function RouterReportsPage({ params }: PageProps) {
  ...
  const totalRev = report?.total_revenue || 0;
  ...
  return (
    <div className="space-y-6">
                  ...
                  <tr key={item.id || idx} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-5 py-3 text-right font-sans">
                      {item.status === "ACTIVE" ? (
                        ...
                        <span className="inline-flex items-center gap-1.5 text-slate-500 dark:text-slate-400 text-[11px] font-bold">
                          <CheckCircle2 className="w-3.5 h-3.5 text-slate-400" />
                          <span>Expiré</span>
                        </span>
                      ) : item.consumed ? (
                        <span className="inline-flex items-center gap-1 text-slate-600 dark:text-slate-300 text-[11px] font-bold">
      ...
      {showPrintModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/80 backdrop-blur-xs p-3 sm:p-4 flex flex-col items-center justify-start py-6 sm:py-4">
          <div className="w-full max-w-4xl bg-white dark:bg-slate-900 rounded-3xl p-4 sm:p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-5 my-auto">
            {/* Header de la modale */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-md shadow-emerald-600/20 flex items-center justify-center gap-1.5 cursor-pointer transition-all"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="border border-slate-200 rounded-xl p-3 bg-slate-50">
                  ...
                  <div className="text-lg font-black text-slate-900 font-mono mt-0.5">
                    {totalRev.toLocaleString()} FCFA
                  </div>
                  <span className="text-[10px] text-slate-500">{totalCount} ventes cumulées</span>
                </div>
              </div>
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 font-mono">
                    {history.length === 0 ? (
                      ...
                      history.slice(0, 20).map((s: any, idx: number) => (
                        <tr key={idx}>
                          ...
                          <td className="px-3 py-2 text-right font-sans text-[11px] font-bold">
                            {s.status === "ACTIVE" ? (
                              <span className="text-emerald-600">En cours</span>
                            ) : s.status === "EXPIRED" ? (
                              <span className="text-slate-500">Expiré</span>
                            ) : (
