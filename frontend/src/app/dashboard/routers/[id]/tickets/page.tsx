...
export default function RouterTicketsPage({ params }: PageProps) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      await api.downloadTicketsPdf(routerId, profile || undefined, ticketIds.length > 0 ? ticketIds : undefined);
      setSuccessMsg("PDF vectoriel Chromium téléchargé avec succès !");
      setTimeout(() => setSuccessMsg(null), 3500);
    } catch (err: any) {
      setErrorMsg("Erreur lors du téléchargement PDF : " + (err.message || "Échec serveur"));
    } finally {
